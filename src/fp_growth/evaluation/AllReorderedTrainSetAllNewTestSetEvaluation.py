...
def evaluate():
    ...
    user_product_data = raw_user_product_data.loc[:, ('user_id', 'product_id')]
    ...
    order_and_product_data = pd.merge(raw_user_product_data, user_product_data,
                                     on=['user_id', 'product_id'],
                                     how='left')

    order_and_product_data = order_and_product_data.loc[:, ['order_id', 'product_id', 'user_id', 'total']]

    order_and_product_data_train_set = order_and_product_data.loc[order_and_product_data['total'] > 1]
    order_and_product_data_test_set = order_and_product_data.loc[order_and_product_data['total'] == 1]


    order_and_product_data = order_and_product_data_train_set.loc[:, ('order_id', 'product_id')];
    ...
    users = order_and_product_data_test_set['user_id'].drop_duplicates()
    ...
    with tqdm(total=len(users)) as progress:
        for user in users:
            ...
            user_products_test_set = list(order_and_product_data_test_set.loc[order_and_product_data_test_set['user_id'] == user]['product_id'])
    ...
    evaluation.to_csv("dataset/modified/fp_growth/evaluation/all-reordered_train-set__all-new_test-set_evaluation--sample-" + str(sample) + "-seed-" + str(seed) + ".csv", index=False)
